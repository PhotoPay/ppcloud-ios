...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (!IS_IOS7_DEVICE) {
        [self setSavedStatusBarStyle:[[UIApplication sharedApplication] statusBarStyle]];
...
- (void)loadScrollViewWithPage:(NSInteger)page {
    ...
    if ([[controller view] superview] == nil) {
        ...
        NSString *labelText = PPLocalize([numberItem valueForKey:nameKey]);
...
- (void)setupBackButtonForPage:(NSInteger)page {
    ...
    if (page <= 0) {
        [_backButton setTitle:PPLocalize(@"PhotoPayHelpClose") forState:UIControlStateNormal];
...
- (void)setupForwardButtonForPage:(NSInteger)page {
    ...
    if (page <= 0) {
        [_forwardButton setTitle:PPLocalize(@"PhotoPayHelpContinue") forState:UIControlStateNormal];
        ...
        [_forwardButton setTitle:PPLocalize(@"PhotoPayHelpClose") forState:UIControlStateNormal];
